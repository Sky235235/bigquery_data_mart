...
from edamodule.QueryConfig import UpdateQuery, DeleteBigQuery
...
def run(start_datetime, end_datetime, table_id, KEY_PATH, sector_table_df):
    ...
    config_query = UpdateQuery(start_datetime, end_datetime)
    ## 앱호출
    print('load_app_df')
    load_start = datetime.now()
    app_query = config_query.update_app_call()
    _app_boarding = load_data(db_info_data, app_query)

    ## 일반주행
    print('load_general_df')
    general_query = config_query.update_general()
    _general_boarding = load_data(db_info_data, general_query)

    ## 예약 취소 쿼리
    print('load_reservation_cancel_df')
    reservation_query = config_query.update_reservation_cancel()
    ...
    print('data_length', len(total_df))
    ...
    print('Delete Original data from bigquery')
    delete_start = datetime.now()
    update_start_datetime = str(list(total_df['reg_datetime'])[0])
    print('update_start_datetime:', update_start_datetime)
    to_datetime = start_datetime

    credentials = service_account.Credentials.from_service_account_file(KEY_PATH)
    client = bigquery.Client(credentials=credentials, project=credentials.project_id)

    _boarding_list = ['앱호출', '일반주행', '예약호출']
    _config_query = DeleteBigQuery(table_id, update_start_datetime, to_datetime)

    for _boarding_name in tqdm(_boarding_list):
        _boarding_df = total_df[total_df['boarding_name'] == _boarding_name].reset_index(drop=True)
        print(f'{_boarding_name}_length:', len(_boarding_df))
        _db_idx_tuple = tuple(_boarding_df['db_idx'].unique())
        if len(_db_idx_tuple) == 0:
            print('nothing to delete')
        else:
            _delete_query = _config_query.delete_data(_db_idx_tuple, _boarding_name)
            _query_job = client.query(_delete_query)
            _query_job.result()

    delete_end = datetime.now()
    print('delete complete, take time:', delete_end - delete_start)

    print('Insert Update data')
    print('update_data_length:', len(total_df))
    insert_start = datetime.now()
    table = client.get_table(table_id)
    # job_config = bigquery.LoadJobConfig(write_disposition='WRITE_TRUNCATE')
    client.load_table_from_dataframe(total_df, table)
    insert_end = datetime.now()
    print('insert complete, take time:', insert_end-insert_start)
